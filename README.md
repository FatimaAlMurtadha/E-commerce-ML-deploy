...
`GET /model-info` returns the checked-in model name, feature list, and recorded test
metrics used by the frontend performance view.

The API uses the checked-in trained model, which is currently SVC. Model selection,
training, feature handling, and prediction belong to the backend. The Streamlit frontend
only collects input, calls the API, and displays the returned result and backend metrics.

`POST /predict` accepts the required session fields and optional time fields. This is the
**request body**; do not send the prediction response back as the next request:
...
The **response** has this shape. These fields are generated by the API and should not be
...
```json
{
  ...
  "reasons": [
    "The customer added 2 item(s) to the cart, a direct intent signal.",
    "The customer made 10 clicks, showing active browsing."
  ],
...
The numeric prediction remains available for integrations. The additional fields explain
which observable session signals supported the score, classify the session as `low`,
`medium`, or `high` purchase intent, and suggest a company action plus a draft customer
message. These are decision-support suggestions, not causal explanations or guarantees.
